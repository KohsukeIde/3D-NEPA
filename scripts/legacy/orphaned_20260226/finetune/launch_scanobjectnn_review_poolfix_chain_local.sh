...
LOG_DIR="${LOG_DIR:-logs/finetune/scan_variants_review_poolfix_chain}"
PIPELINE_LOG="${PIPELINE_LOG:-${LOG_DIR}/pipeline.log}"
PIPELINE_PID="${PIPELINE_PID:-${LOG_DIR}/pipeline.pid}"

mkdir -p "${LOG_DIR}"

nohup bash scripts/finetune/run_scanobjectnn_review_poolfix_chain_local.sh > "${PIPELINE_LOG}" 2>&1 &
echo $! > "${PIPELINE_PID}"
