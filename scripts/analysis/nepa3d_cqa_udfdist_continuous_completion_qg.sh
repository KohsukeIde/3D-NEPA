...
RUN_TAG="${RUN_TAG:-cqa_udfdist_continuous_completion_$(date +%Y%m%d_%H%M%S)}"
CKPT="${CKPT:?set CKPT=...}"
...
echo "=== CQA UDFDIST CONTINUOUS COMPLETION ===" | tee "${LOG_PATH}"
...
echo "grid_res=${GRID_RES} max_shapes=${MAX_SHAPES} mesh_eval=${MESH_EVAL}" | tee -a "${LOG_PATH}"
echo "export_assets=${EXPORT_ASSETS} assets_root=${ASSETS_ROOT}" | tee -a "${LOG_PATH}"
echo "out_json=${OUT_JSON}" | tee -a "${LOG_PATH}"
echo | tee -a "${LOG_PATH}"

python -u -m nepa3d.tracks.patch_nepa.cqa.analysis.completion_udfdist_continuous \
  --ckpt "${CKPT}" \
