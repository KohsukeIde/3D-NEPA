...
RUN_TAG="${RUN_TAG:-cqa_multitype_continuous_suite_$(date +%Y%m%d_%H%M%S)}"
CKPT="${CKPT:?set CKPT=...}"
...
echo "=== CQA MULTITYPE CONTINUOUS SUITE ===" | tee "${LOG_PATH}"
...
echo "tau=${TAU}" | tee -a "${LOG_PATH}"
echo "out_json=${OUT_JSON}" | tee -a "${LOG_PATH}"
echo | tee -a "${LOG_PATH}"

python -u -m nepa3d.tracks.patch_nepa.cqa.analysis.eval_multitype_continuous_suite \
  --ckpt "${CKPT}" \
