...
# Keep same ON-feature setting for fair comparison.
...
# Model-1: reuse causal plus-gut checkpoint as reference (skip train if exists).
export MODEL1_TAG="${MODEL1_TAG:-nepa_impl_causal_plusgut_ref_s0_e50}"
...
# Model-2: bbox topology variant under same +grad/+unc conditions.
export MODEL2_TAG="${MODEL2_TAG:-nepa_impl_encdec_plusgut_bbox_s0_e50}"
export MODEL2_SAVE_DIR="${MODEL2_SAVE_DIR:-runs/eccv_upmix_nepa_impl_encdec_plusgut_bbox_s0}"
export MODEL2_ARCH="${MODEL2_ARCH:-encdec}"
export MODEL2_QA_LAYOUT="${MODEL2_QA_LAYOUT:-split}"
export MODEL2_TOPO_K="${MODEL2_TOPO_K:-16}"
export MODEL2_TOPO_RAY_COORD="${MODEL2_TOPO_RAY_COORD:-bbox}"
