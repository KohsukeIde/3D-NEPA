#!/bin/bash
#
# Multi-node launcher for classification/CPAC eval (Accelerate DDP via pbsdsh).
#
# Example:
#   qsub -v RUN_TAG=runA,CKPT=/path/to/last.pt,RUN_SCAN=1,RUN_MODELNET=0,RUN_CPAC=0 \
#     scripts/eval/nepa3d_eval_cls_cpac_multinode_pbsdsh.sh
#
#PBS -l rt_QF=4
#PBS -l walltime=24:00:00
#PBS -j oe
#PBS -N nepa3d_eval_ddp
#PBS -o nepa3d_eval_ddp.out
#PBS -e nepa3d_eval_ddp.err
...
RUN_TAG="${RUN_TAG:-eval}"
DDP_LOG_ROOT="${DDP_LOG_ROOT:-${WORKDIR}/logs/ddp_eval}"
...
RUN_DIR="${DDP_LOG_ROOT}/ddp_eval_${PBS_JOBID:-manual}_${RUN_TAG}"
...
host="$(hostname)"
log="${RUN_DIR}/logs/${host}.eval.log"
exec > "${log}" 2>&1
...
exec env \
  ...
  bash "${WORKDIR}/scripts/eval/nepa3d_eval_cls_cpac_qf.sh"
SH
chmod +x "${NODE_ENTRY}"
