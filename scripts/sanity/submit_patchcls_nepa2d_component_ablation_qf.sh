...
RUN_SCRIPT="${WORKDIR}/scripts/finetune/patchcls_scanobjectnn_scratch.sh"
...
RUN_SET="${RUN_SET:-patchcls_nepa2d_components_$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="${LOG_ROOT:-${WORKDIR}/logs/sanity/patchcls/${RUN_SET}}"
mkdir -p "${LOG_ROOT}"

VARIANTS="${VARIANTS:-obj_bg}"  # comma-separated: obj_bg,obj_only,pb_t50_rs
...
MODEL_SOURCE="${MODEL_SOURCE:-patchcls}"
...
# label qk_norm layerscale_value rope_theta
COMPONENT_ROWS=(
  "off 0 0.0 0.0"
  "rope_only 0 0.0 100.0"
  "qk_only 1 0.0 0.0"
  "layerscale_only 0 1e-5 0.0"
  "full 1 1e-5 100.0"
)
...
for variant in "${_variants[@]}"; do
  ...
  for row in "${COMPONENT_ROWS[@]}"; do
    read -r label qk_norm layerscale_value rope_theta <<< "${row}"
    run_tag="${variant}_${label}_${RUN_SET}"
    run_name="patchcls_${variant}_${label}_${RUN_SET}"
    out="${LOG_ROOT}/${variant}_${label}.out"
    err="${LOG_ROOT}/${variant}_${label}.err"
    job_name="pc_${variant}_${label}"
    ...
    jid="$("${cmd[@]}")"
    echo "${jid} ${variant} ${label} qk=${qk_norm} ls=${layerscale_value} rope=${rope_theta} ${run_tag}" | tee -a "${job_ids_file}"
    echo "[submitted] ${jid} variant=${variant} label=${label}"
  done
done
