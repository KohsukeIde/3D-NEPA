...
variants=(official official nomask nomask)
seeds=(1 2 1 2)
...
case "${variant}" in
  official)
    ...
    run_name="pgpt_s_shapenetpart_official_e300_seed${seed}"
    ;;
  nomask)
    ckpt="${POINTGPT_DIR}/experiments/pretrain_nomask/PointGPT-S/pgpt_s_nomask_e300/ckpt-last.pth"
    run_name="pgpt_s_shapenetpart_nomask_e300_seed${seed}"
    ;;
  *)
...
echo "=== PointGPT-S ShapeNetPart seed repeat ==="
echo "date=$(date -Is)"
echo "host=$(hostname)"
