...
CKPT_PATH="${CKPT_PATH:-${POINTGPT_DIR}/experiments/finetune_scan_objbg/PointGPT-S/pgpt_s_nomask_ordrand_objbg_e300/ckpt-best.pth}"
OUT_PREFIX="${OUT_PREFIX:-${WORKDIR}/results/ptgpt_nomask_ordrand_objbg}"

cd "${POINTGPT_DIR}"

echo "=== PointGPT no-mask order-randomized obj_bg audits ==="
echo "date=$(date -Is)"
...
echo "[done] PointGPT no-mask order-randomized obj_bg audits"
