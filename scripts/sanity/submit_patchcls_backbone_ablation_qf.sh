...
for variant in "${_variants[@]}"; do
  ...
  for mode in "${_modes[@]}"; do
    ...
    [[ -z "${mode}" ]] && continue
    if [[ "${mode}" != "vanilla" && "${mode}" != "nepa2d" ]]; then
      echo "[error] unknown mode: ${mode} (use vanilla|nepa2d)"
      exit 2
    fi

    run_tag="${variant}_${mode}_${RUN_SET}"
    run_name="patchcls_${variant}_${mode}_${RUN_SET}"
    out="${LOG_ROOT}/${variant}_${mode}.out"
    err="${LOG_ROOT}/${variant}_${mode}.err"
    job_name="pc_${variant}_${mode}"
    ...
    jid="$("${cmd[@]}")"
    echo "${jid} ${variant} ${mode} ${run_tag}" | tee -a "${job_ids_file}"
    echo "[submitted] ${jid} variant=${variant} mode=${mode}"
  done
done
