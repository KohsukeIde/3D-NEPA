...
RUN_SET="${RUN_SET:-patchnepa_cqa_distnorm_shared_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${RUN_TAG:-cqa_distnorm_independent_g2_s10000}"
...
export SAVE_DIR
export LOG_ROOT
export MIX_CONFIG="${MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_dist_norm.yaml}"
...
export EVAL_TASK_FILTER="${EVAL_TASK_FILTER:-udf_distance,mesh_normal}"
...
export WANDB_TAGS="${WANDB_TAGS:-abci,cqa,multitype,distnorm,independent}"
