...
export RUN_SET="${RUN_SET:-patchnepa_cqa_udfdist_curve_selfq_$(date +%Y%m%d_%H%M%S)}"
export RUN_TAG="${RUN_TAG:-cqa_udfdist_worldv3_independent_selfq_g2_s10000}"
...
export QUERY_INTERFACE_MODE="self_q"
export WANDB_PROJECT="${WANDB_PROJECT:-patchnepa-cqa-curve}"
export WANDB_TAGS="${WANDB_TAGS:-abci,cqa,curve,udf_distance,world_v3,independent,self_q}"
