...
RUN_SET="${RUN_SET:-patchnepa_cqa_v2_distnorm_unsigned_prefixlm_${STAMP}}"
TRAIN_RUN_TAG="${TRAIN_RUN_TAG:-cqa_v2_distnorm_unsigned_prefixlm_independent_g2_s10000}"
...
train_out="$(
  env \
    ...
    SEED="${SEED:-0}" \
    WANDB_TAGS="${WANDB_TAGS:-abci,cqa,cqa_v2,distnorm_unsigned,prefixlm,independent}" \
...
SUITE_RUN_TAG="${SUITE_RUN_TAG:-cqa_v2_distnorm_unsigned_prefixlm_suite}"
...
for mode in same offdiag; do
  ...
  comp_run_tag="cqa_v2_distnorm_unsigned_prefixlm_${mode}_translation_g16_s64"
  comp_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    MODE="${mode}" \
...
for variant in obj_bg obj_only pb_t50_rs; do
  ...
  cls_run_tag="cqa_v2_distnorm_unsigned_prefixlm_${variant}_seed${SEED:-0}"
  cls_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    VARIANT="${variant}" \
