...
SCRIPT="${ROOT_DIR}/scripts/analysis/nepa3d_cqa_udfdist_continuous_completion_qg.sh"
...
case "${MODE}" in
  same) DEFAULT_MIX_CONFIG="nepa3d/configs/shapenet_unpaired_mix_v2_cqa_udfdist_continuous.yaml" ;;
  offdiag) DEFAULT_MIX_CONFIG="nepa3d/configs/shapenet_unpaired_mix_v2_cqa_udfdist_continuous_pcbank.yaml" ;;
  *)
...
RUN_SET="${RUN_SET:-patchnepa_cqa_udfdist_continuous_completion_${MODE}_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${RUN_TAG:-cqa_udfdist_continuous_${MODE}_translation_g${GRID_RES:-16}_s${MAX_SHAPES:-64}}"
