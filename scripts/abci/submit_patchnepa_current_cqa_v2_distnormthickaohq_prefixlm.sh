...
RUN_SET="${RUN_SET:-patchnepa_cqa_v2_distnormthickaohq_prefixlm_${STAMP}}"
TRAIN_RUN_TAG="${TRAIN_RUN_TAG:-cqa_v2_distnormthickaohq_prefixlm_independent_g2_s10000}"
...
train_out="$(
  env \
    ...
    MIX_CONFIG="${MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_v2_dist_norm_thick_aohq.yaml}" \
    ...
    EVAL_TASK_FILTER="${EVAL_TASK_FILTER:-udf_distance,mesh_normal_unsigned,udf_thickness_valid_qbin,mesh_ao_hq}" \
    ...
    SEED="${SEED:-0}" \
    WANDB_TAGS="${WANDB_TAGS:-abci,cqa,cqa_v2,distnormthickaohq,prefixlm,independent}" \
...
SUITE_RUN_TAG="${SUITE_RUN_TAG:-cqa_v2_distnormthickaohq_prefixlm_suite}"
...
suite_out="$(
  ...
  SAME_MIX_CONFIG="${SAME_MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_v2_dist_norm_thick_aohq.yaml}" \
  OFFDIAG_MIX_CONFIG="${OFFDIAG_MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_v2_dist_norm_thick_aohq_pcbank_eval.yaml}" \
  TASKS="${TASKS:-udf_distance,mesh_normal_unsigned,udf_thickness_valid_qbin,mesh_ao_hq}" \
...
for mode in same offdiag; do
  ...
  comp_run_tag="cqa_v2_distnormthickaohq_prefixlm_${mode}_translation_g16_s64"
  comp_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    MODE="${mode}" \
...
for variant in obj_bg obj_only pb_t50_rs; do
  ...
  cls_run_tag="cqa_v2_distnormthickaohq_prefixlm_${variant}_seed${SEED:-0}"
  cls_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    VARIANT="${variant}" \
