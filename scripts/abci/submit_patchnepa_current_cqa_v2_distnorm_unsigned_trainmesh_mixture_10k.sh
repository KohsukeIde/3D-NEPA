...
RUN_SET="${RUN_SET:-patchnepa_cqa_v2_distnorm_unsigned_trainmesh_mixture10k_${STAMP}}"
TRAIN_RUN_TAG="${TRAIN_RUN_TAG:-cqa_v2_distnorm_unsigned_trainmesh_mixture10k_independent_g2_s10000}"
...
train_out="$(
  env \
    ...
    WANDB_TAGS="${WANDB_TAGS:-abci,cqa,cqa_v2,distnorm_unsigned,trainmesh_common,mixture,protocol_isolation}" \
...
SUITE_RUN_TAG="${SUITE_RUN_TAG:-cqa_v2_distnorm_unsigned_trainmesh_mixture10k_suite}"
...
for mode in same offdiag; do
  ...
  comp_run_tag="cqa_v2_distnorm_unsigned_trainmesh_mixture10k_${mode}_translation_g16_s64"
  comp_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    MODE="${mode}" \
