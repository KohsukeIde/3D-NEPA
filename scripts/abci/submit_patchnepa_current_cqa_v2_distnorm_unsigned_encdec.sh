...
RUN_SET="${RUN_SET:-patchnepa_cqa_v2_distnorm_unsigned_encdec_${STAMP}}"
TRAIN_RUN_TAG="${TRAIN_RUN_TAG:-cqa_v2_distnorm_unsigned_encdec_independent_d4_g2_s10000}"
...
train_out="$(
  env \
    ...
    MIX_CONFIG="${MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_v2_dist_norm_unsigned.yaml}" \
    CODEC_VERSION="${CODEC_VERSION:-cqa_v2}" \
    MODEL_ARCH="${MODEL_ARCH:-encdec}" \
    ...
    SEED="${SEED:-0}" \
    WANDB_TAGS="${WANDB_TAGS:-abci,cqa,cqa_v2,distnorm_unsigned,encdec,independent}" \
...
SUITE_RUN_TAG="${SUITE_RUN_TAG:-cqa_v2_distnorm_unsigned_encdec_suite}"
...
for mode in same offdiag; do
  ...
  comp_run_tag="cqa_v2_distnorm_unsigned_encdec_${mode}_translation_g16_s64"
  comp_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    MODE="${mode}" \
...
for variant in obj_bg obj_only pb_t50_rs; do
  ...
  cls_run_tag="cqa_v2_distnorm_unsigned_encdec_${variant}_seed${SEED:-0}"
  cls_out="$(
    QSUB_DEPEND="afterok:${train_job}" \
    CKPT="${CKPT_PATH}" \
    VARIANT="${variant}" \
