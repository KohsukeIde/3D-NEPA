...
RUN_SET="${RUN_SET:-patchnepa_geo_teacher_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${RUN_TAG:-geo_teacher_distnorm_packed_mh_100ep}"
...
export WANDB_TAGS="${WANDB_TAGS:-abci,cqa,geo_teacher,route_a,matched_compare}"
...
echo "[abci-geo-teacher] mix_config=${MIX_CONFIG}"
echo "[abci-geo-teacher] save_dir=${SAVE_DIR}"
echo "[abci-geo-teacher] epochs=${EPOCHS} batch=${BATCH}"
echo "[abci-geo-teacher] model_arch=${MODEL_ARCH} head_mode=${HEAD_MODE} sampling_protocol=${SAMPLING_PROTOCOL}"
echo "[abci-geo-teacher] factorization=${ANSWER_FACTORIZATION} query_interface=${QUERY_INTERFACE_MODE} loss_balance=${LOSS_BALANCE}"
