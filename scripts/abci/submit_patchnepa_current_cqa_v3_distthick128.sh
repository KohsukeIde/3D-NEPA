...
RUN_SET="${RUN_SET:-patchnepa_cqa_v3_distthick128_$(date +%Y%m%d_%H%M%S)}"
RUN_TAG="${RUN_TAG:-cqa_v3_distthick128_independent_g2_s10000}"
...
export SAVE_DIR
export LOG_ROOT
export MIX_CONFIG="${MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_v3_dist_thick.yaml}"
...
export SEED="${SEED:-0}"
export EVAL_TASK_FILTER="${EVAL_TASK_FILTER:-udf_distance,udf_thickness_valid_qbin}"
...
export WANDB_TAGS="${WANDB_TAGS:-abci,cqa,cqa_v3,distthick128,independent}"
...
SUITE_RUN_TAG="${SUITE_RUN_TAG:-cqa_v3_distthick128_suite}"
...
suite_out="$(
  ...
  OFFDIAG_MIX_CONFIG="${OFFDIAG_MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_v3_dist_thick_pcbank_eval.yaml}" \
  TASKS="${TASKS:-udf_distance,udf_thickness_valid_qbin}" \
