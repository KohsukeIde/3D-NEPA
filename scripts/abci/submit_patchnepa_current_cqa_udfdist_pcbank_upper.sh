...
export RUN_SET="${RUN_SET:-patchnepa_cqa_udfdist_pcbank_upper_$(date +%Y%m%d_%H%M%S)}"
export RUN_TAG="${RUN_TAG:-cqa_udfdist_pcbank_upper_g2_s10000}"
export MIX_CONFIG="${MIX_CONFIG:-nepa3d/configs/shapenet_unpaired_mix_v2_cqa_udfdist_pcbank.yaml}"
...
export WANDB_PROJECT="${WANDB_PROJECT:-patchnepa-cqa-pcbank-upper}"
export WANDB_GROUP="${WANDB_GROUP:-${RUN_SET}}"
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-${RUN_TAG}}"
export WANDB_TAGS="${WANDB_TAGS:-abci,cqa,pcbank,upper,udf_distance,world_v3}"
