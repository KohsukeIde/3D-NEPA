...
RUN_TAG="${RUN_TAG:-cqa_v2_distnorm_unsigned_independent_g2}"
...
export WANDB_TAGS="${WANDB_TAGS:-abci,cqa,current,dist,norm_unsigned,cqa_v2}"
...
export CODEC_VERSION="${CODEC_VERSION:-}"
...
echo "[abci-cqa-pretrain] model_arch=${MODEL_ARCH} decoder_layers=${DECODER_LAYERS}"
echo "[abci-cqa-pretrain] external_ckpt=${EXTERNAL_BACKBONE_CKPT:-<none>} freeze_external=${FREEZE_EXTERNAL_ENCODER}"
...
echo "[abci-cqa-pretrain] head_mode=${HEAD_MODE}"
echo "[abci-cqa-pretrain] sampling_protocol=${SAMPLING_PROTOCOL}"
echo "[abci-cqa-pretrain] query_order=${QUERY_ORDER} eval_query_order=${EVAL_QUERY_ORDER}"
echo "[abci-cqa-pretrain] codec_version=${CODEC_VERSION:-<infer>}"
