#!/bin/bash
#
# Multi-node launcher for patch-NEPA-tokens pretrain (Accelerate DDP via per-node pbsdsh).
#
# Example (8 GPUs total = 2 nodes x 4 gpus/node):
#   qsub -l rt_QF=2 -l walltime=24:00:00 \
#     -v RUN_TAG=patchnepa_tokens_ddp8,SAVE_DIR=runs/patchnepa_tokens/patchnepa_tokens_ddp8 \
...
RUN_TAG="${RUN_TAG:-patchnepa_tokens_ddp}"
DDP_LOG_ROOT="${DDP_LOG_ROOT:-${WORKDIR}/logs/ddp_patch_nepa_tokens_pretrain}"
if [[ "${DDP_LOG_ROOT}" != /* ]]; then
  DDP_LOG_ROOT="${WORKDIR}/${DDP_LOG_ROOT}"
fi
...
RUN_DIR="${DDP_LOG_ROOT}/ddp_patchnepa_tokens_${PBS_JOBID:-manual}_${RUN_TAG}"
mkdir -p "${RUN_DIR}/logs"
PRETRAIN_DONE_MARKER="${RUN_DIR}/pretrain_done.marker"
LAUNCH_SCRIPT_SRC="${WORKDIR}/scripts/pretrain/nepa3d_pretrain_patch_nepa_tokens_qf.sh"
LAUNCH_SCRIPT_RUN="${RUN_DIR}/nepa3d_pretrain_patch_nepa_tokens_qf.snapshot.sh"
...
echo "=== PATCH-NEPA TOKENS DDP TOPOLOGY ==="
...
PROPAGATE_VARS=(
  ...
  MAX_STEPS SAVE_EVERY
  N_SURF N_QRY
  TOKEN_QA_LAYOUT
  PM_PC_NORM PM_SCALE_TRANSLATE PM_SCALE_LOW PM_SCALE_HIGH PM_TRANSLATE PM_TRANSFORM_ANSWERS
  ANSWER_IN_DIM
  ANSWER_MLP_LAYERS ANSWER_POOL
  PRETRAIN_OBJECTIVE INFONCE_TAU RECON_CTX_WEIGHT RECON_Q_WEIGHT RECON_A_WEIGHT RECON_LOSS_MODE RECON_CHAMFER_METRIC
  RECON_GENERATOR_DEPTH
  CENTER_TARGET_ALPHA
  REG_VAR_WEIGHT REG_COV_WEIGHT REG_VAR_GAMMA REG_VAR_EPS REG_SCOPE REG_SOURCE
  LOSS_MASK_MODE SKIP_K NEPA_CENTER_MODE NEPA_CENTER_WARMUP_FRAC
  DUAL_MASK_MODE DUAL_MASK_KEEP_PREFIX DUAL_MASK_COLUMN_RATIO
  ...
  DUAL_MASK_NEAR DUAL_MASK_FAR DUAL_MASK_WINDOW DUAL_MASK_TYPE_AWARE DUAL_MASK_COLUMN_RATIO DUAL_MASK_WARMUP_FRAC
...
host="$(hostname)"
log="${RUN_DIR}/logs/${host}.patchnepa_tokens.log"
exec > "${log}" 2>&1
...
echo "=== PATCH-NEPA TOKENS NODE ENTRY ==="
echo "host=${host}"
