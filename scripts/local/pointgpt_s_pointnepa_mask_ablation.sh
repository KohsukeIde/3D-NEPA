...
RUN_TAG="${RUN_TAG:-pointnepa_s_maskoff_$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="${LOG_ROOT:-${WORKDIR}/logs/local/pointgpt_s_pointnepa_mask_ablation}"
POLL_SEC="${POLL_SEC:-60}"
NUM_WORKERS="${NUM_WORKERS:-8}"
NPROC_PER_NODE="${NPROC_PER_NODE:-2}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1}"

POINTNEPA_ON_SUMMARY_PATH="${POINTNEPA_ON_SUMMARY_PATH:-${WORKDIR}/logs/local/pointgpt_s_ft_recipe_matrix_2x2/nepa_cosine_clsonly_pointgpt_s_ft_recipe_matrix_2x2_20260318_summary.md}"
POINTNEPA_CONFIG_PATH="${POINTNEPA_CONFIG_PATH:-cfgs/PointGPT-S/pretrain_nepa_cosine_shapenet_cache_v0_nomask.yaml}"
POINTNEPA_EXP_NAME="${POINTNEPA_EXP_NAME:-pointgpt_s_nepa_cosine_shapenet_cache_v0_nomask_${RUN_TAG}}"
...
ensure_pointnepa_pretrain() {
  ...
  if (( epoch >= POINTNEPA_MAX_EPOCH )); then
    echo "[done] pointnepa_pretrain already complete: epoch=${epoch}" >&2
    printf '%s\n' "${ckpt_path}"
    return 0
  fi

  if pgrep -af -- "--exp_name ${POINTNEPA_EXP_NAME}" >/dev/null; then
    wait_for_exp_completion "pointnepa_pretrain_existing" "${POINTNEPA_EXP_NAME}" "${exp_path}" "${POINTNEPA_MAX_EPOCH}" >&2
    printf '%s\n' "${ckpt_path}"
    return 0
  fi
  ...
  if [[ -f "${ckpt_path}" ]]; then
    extra_args="--resume"
    echo "[resume] pointnepa_pretrain: epoch=${epoch} exp_name=${POINTNEPA_EXP_NAME}" >&2
  else
    echo "[launch] pointnepa_pretrain: exp_name=${POINTNEPA_EXP_NAME}" >&2
  fi
  ...
  wait_for_exp_completion "pointnepa_pretrain" "${POINTNEPA_EXP_NAME}" "${exp_path}" "${POINTNEPA_MAX_EPOCH}" >&2
  printf '%s\n' "${ckpt_path}"
}
...
POINTNEPA_CKPT_PATH="$(ensure_pointnepa_pretrain)"

OBJ_BG_EXP_NAME="pointgpt_ft_objbg_from_pointnepa_maskoff_clsonly_${RUN_TAG}"
OBJ_ONLY_EXP_NAME="pointgpt_ft_objonly_from_pointnepa_maskoff_clsonly_${RUN_TAG}"
HARDEST_EXP_NAME="pointgpt_ft_hardest_from_pointnepa_maskoff_clsonly_${RUN_TAG}"
...
ensure_finetune_stage \
  "pointnepa_maskoff_clsonly_objbg" \
  "${OBJ_BG_CONFIG_PATH}" \
  "${OBJ_BG_EXP_NAME}" \
  "${OBJ_BG_EXP_PATH}" \
  "${FT_MAX_EPOCH_OBJ_BG}" \
  "${POINTNEPA_CKPT_PATH}" \
  "pointgpt,s,finetune,objbg,pointnepa,maskoff,clsonly" \
  "29673"

ensure_finetune_stage \
  "pointnepa_maskoff_clsonly_objonly" \
  "${OBJ_ONLY_CONFIG_PATH}" \
  "${OBJ_ONLY_EXP_NAME}" \
  "${OBJ_ONLY_EXP_PATH}" \
  "${FT_MAX_EPOCH_OBJ_ONLY}" \
  "${POINTNEPA_CKPT_PATH}" \
  "pointgpt,s,finetune,objonly,pointnepa,maskoff,clsonly" \
  "29675"

ensure_finetune_stage \
  "pointnepa_maskoff_clsonly_hardest" \
  "${HARDEST_CONFIG_PATH}" \
  "${HARDEST_EXP_NAME}" \
  "${HARDEST_EXP_PATH}" \
  "${FT_MAX_EPOCH_HARDEST}" \
  "${POINTNEPA_CKPT_PATH}" \
  "pointgpt,s,finetune,hardest,pointnepa,maskoff,clsonly" \
  "29677"

SUMMARY_PATH="${LOG_ROOT}/${RUN_TAG}_summary.md"
cat > "${SUMMARY_PATH}" <<EOF
# PointNEPA-S Mask-Off Summary

- run_tag: \`${RUN_TAG}\`
- baseline_mask_on_summary: \`${POINTNEPA_ON_SUMMARY_PATH}\`
- source_label: \`pointnepa_maskoff\`
- pretrain_config: \`${POINTNEPA_CONFIG_PATH}\`
- mask_ratio: \`0.0\`
- note: \`causal next-token mask remains enabled; only PointGPT column mask is removed\`
