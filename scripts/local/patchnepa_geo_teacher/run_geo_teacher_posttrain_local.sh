...
EXPECTED_CKPT_PATH="${PRETRAIN_SAVE_DIR}/${CKPT_NAME}"
if [[ "${STRICT_PRETRAIN_TAG_CKPT_MATCH:-1}" == "1" && "${CKPT_PATH}" != "${EXPECTED_CKPT_PATH}" ]]; then
  echo "[warn] CKPT_PATH (${CKPT_PATH}) mismatched PRETRAIN_RUN_TAG (${PRETRAIN_RUN_TAG}); overriding to ${EXPECTED_CKPT_PATH}"
  export CKPT_PATH="${EXPECTED_CKPT_PATH}"
fi
