...
PRETRAIN_EXP="${PRETRAIN_EXP:-pgpt_s_masked_ordrand_e300_20260506}"
PRETRAIN_CONFIG="${PRETRAIN_CONFIG:-cfgs/PointGPT-S/pretrain_orderrandom.yaml}"
...
run_pretrain() {
  ...
  run_logged \
    ...
    "${LOG_ROOT}/pretrain_masked_orderrandom.log" \
    env CUDA_VISIBLE_DEVICES="${PRETRAIN_GPUS}" \
      torchrun --standalone --nproc_per_node="${PRETRAIN_NPROC}" --master_port=29606 \
      main.py \
      --launcher pytorch \
...
scan_exp() {
  case "$1" in
    obj_bg) printf "pgpt_s_masked_ordrand_objbg_e300_20260506" ;;
    obj_only) printf "pgpt_s_masked_ordrand_objonly_e300_20260506" ;;
    pb_t50_rs) printf "pgpt_s_masked_ordrand_hardest_e300_20260506" ;;
    *) echo "[error] unknown ScanObjectNN variant: $1" >&2; return 2 ;;
  esac
}
...
scan_port() {
  case "$1" in
    obj_bg) printf "29611" ;;
    obj_only) printf "29612" ;;
    pb_t50_rs) printf "29613" ;;
  esac
}
...
run_scan_ft_one() {
  local variant="$1"
  local gpu="$2"
  local cfg exp ckpt marker logfile port
  cfg="$(scan_config "${variant}")"
  exp="$(scan_exp "${variant}")"
  ckpt="$(scan_ckpt "${variant}")"
  port="$(scan_port "${variant}")"
...
PART_EXP="${PART_EXP:-pgpt_s_shapenetpart_masked_ordrand_e300_20260506}"
...
audit_scan_one() {
  ...
  cfg="$(scan_config "${variant}")"
  ckpt="$(scan_ckpt "${variant}")"
  prefix="${RUN_ROOT}/scanobjectnn_${variant}_masked_ordrand"
...
audit_part() {
  ...
  prefix="${RUN_ROOT}/shapenetpart_masked_ordrand"
...
def scan_readout_row(split: str):
    p = run_root / f"scanobjectnn_{split}_masked_ordrand_readout.json"
    d = read_json(p)
    if not d:
        return None
...
def scan_support_row(split: str):
    p = run_root / f"scanobjectnn_{split}_masked_ordrand_support.json"
    d = read_json(p)
    if not d:
        return None
...
lines = [
    "# PointGPT-S Mask-On Order-Randomized Diagnostics",
    "",
    "Purpose: fill the missing mask-on + order-randomized PointGPT-S cells for ScanObjectNN and ShapeNetPart with the current local code and unique-retained ShapeNetPart scoring.",
    "",
    "## Checkpoints",
    "",
...
for split in ["obj_bg", "obj_only", "pb_t50_rs"]:
    prefix = f"scanobjectnn_{split}_masked_ordrand"
    lines.extend([
...
lines.extend([
    f"- ShapeNetPart support: `{run_root / 'shapenetpart_masked_ordrand_support_unique.md'}`",
    f"- ShapeNetPart eval-time grouping: `{run_root / 'shapenetpart_masked_ordrand_grouping_unique.md'}`",
    "",
...
(run_root / "pointgpt_masked_orderrandom_diagnostics_summary.md").write_text("\n".join(lines) + "\n")
PY
}
...
echo "[done] PointGPT-S mask-on order-randomized diagnostics"
echo "summary=${RUN_ROOT}/pointgpt_masked_orderrandom_diagnostics_summary.md"
