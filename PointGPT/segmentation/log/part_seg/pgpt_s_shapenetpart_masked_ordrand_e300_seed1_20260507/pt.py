...
try:
    ...
    module_path = ROOT / "extensions" / "chamfer_dist" / "__init__.py"
    spec = importlib.util.spec_from_file_location("pointgpt_root_chamfer_dist", module_path)
    if spec is None or spec.loader is None:
        raise
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    ChamferDistanceL1 = module.ChamferDistanceL1
    ChamferDistanceL2 = module.ChamferDistanceL2
import numpy as np
from z_order import *
...
class Group(nn.Module):
    def __init__(self, num_group, group_size, group_mode="fps_knn", group_radius=0.22, group_voxel_grid=6):
        super().__init__()
    ...
    def _voxel_centers(self, xyz):
        ...
        for b in range(batch_size):
            ...
            if chosen_idx.numel() >= self.num_group:
                selected = fps(pts[chosen_idx].unsqueeze(0), self.num_group).squeeze(0)
            else:
                fill = fps(pts.unsqueeze(0), self.num_group - chosen_idx.numel()).squeeze(0)
    ...
    def _random_group_indices(self, xyz):
        ...
        for _ in range(batch_size):
            rows.append(torch.randint(0, num_points, (self.num_group, self.group_size), device=xyz.device))
        return torch.stack(rows, dim=0)
...
class get_model(nn.Module):
    def __init__(self, cls_dim, trans_dim=384, depth=12, drop_path_rate=0.1, num_heads=6, decoder_depth=4, group_size=32, num_group=128, prop_dim=1024, label_dim1=512, label_dim2=256, encoder_dims=384, group_mode="fps_knn", group_radius=0.22, group_voxel_grid=6):
        super().__init__()
        ...
        self.group_voxel_grid = group_voxel_grid
