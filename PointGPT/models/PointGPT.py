...
def _load_chamfer_losses():
    from extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL2

    return ChamferDistanceL1, ChamferDistanceL2
...
class Group(nn.Module):
    ...
    def build_sorted_indices(self, xyz, center):
        ...
        if self.order_mode in {"fixed_random", "stable_random"}:
...
class GPT_Transformer(nn.Module):
    ...
    def forward(self, neighborhood, center, noaug=False, classify=False, return_patch_tokens=False, prediction_mode="pointgpt"):
        # generate mask
        ...
        if classify == False:
            encoded_features = self.blocks(
                group_input_tokens, pos_absolute, attn_mask, classify=classify, prediction_mode=prediction_mode)
            if return_patch_tokens:
                return encoded_features, group_input_tokens
...
class PointGPT(nn.Module):
    ...
    def build_loss_func(self, loss_type):
        self.loss_func_p = None
        self.loss_func_p1 = None
        self.loss_func_p2 = None
        if loss_type == "cdl1":
            ChamferDistanceL1, _ = _load_chamfer_losses()
            self.loss_func_p = ChamferDistanceL1().cuda()
        elif loss_type == 'cdl2':
            _, ChamferDistanceL2 = _load_chamfer_losses()
            self.loss_func_p = ChamferDistanceL2().cuda()
        elif loss_type == 'cdl12':
            ChamferDistanceL1, ChamferDistanceL2 = _load_chamfer_losses()
            self.loss_func_p1 = ChamferDistanceL1().cuda()
            self.loss_func_p2 = ChamferDistanceL2().cuda()
        elif loss_type in ["nepa_cosine", "nepa_cosine_vitshift"]:
            pass
        else:
    ...
    def _compute_nepa_cosine_loss_and_diag(self, pred_tokens, target_tokens):
        ...
        if pred_tokens.size(1) > 1:
            ...
            cos_prev_mean = cos_prev.mean()
            gap = cos_tgt_mean - cos_prev_mean
            copy_win = (cos_prev >= cos_tgt).float().mean()
        else:
            cos_tgt_mean = cos_full.mean()
            cos_prev_mean = None
            gap = None
            copy_win = None

        return loss_main, self._build_pretrain_diag(
            loss_main=loss_main,
            cos_tgt=cos_tgt_mean,
    ...
    def forward(self, pts, vis=False, **kwargs):
        ...
        if self.loss == "nepa_cosine":
            if vis:
                raise NotImplementedError("vis=True is only supported for reconstruction objectives.")
            # GPT_extractor already applies the one-step causal shift. The current-token
