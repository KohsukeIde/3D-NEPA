...
def run_net(args, config, train_writer=None, val_writer=None, wandb_run=None):
    ...
    ft_recon_weight = float(getattr(args, 'ft_recon_weight', 3.0))
    compute_recon = ft_recon_weight > 0.0
    # build dataset
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            ret, loss1 = base_model(points, compute_recon=compute_recon)

            loss, acc = base_model.module.get_loss_acc(ret, label)

            _loss = loss + ft_recon_weight * loss1
            ...
            if wandb_run is not None and (n_itr % wandb_log_every == 0):
                wb = {
                    'train/loss_cls': float(loss.item()),
                    'train/loss_recon': float(loss1.item()),
                    'train/acc': float(acc.item()),
        ...
        if wandb_run is not None:
            wandb_run.log(
                {
                    'epoch/loss_cls': float(losses.avg(0)),
                    'epoch/loss_recon': float(losses.avg(1)),
                    'epoch/acc': float(losses.avg(2)),
            ...
            if args.vote:
                if metrics.acc > 92.1 or (better and metrics.acc > 91):
                    ...
                    if wandb_run is not None:
                        wandb_run.log(
                            {
                                'val/acc_vote': float(metrics_vote.acc),
                                'val/epoch': int(epoch),
                            },
                            step=int((epoch + 1) * max(1, n_batches)),
                        )
        ...
        if int(getattr(args, 'save_last_every_epoch', 1)) == 1:
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            logits, loss1 = base_model(points, compute_recon=False)
            target = label.view(-1)
...
def test(base_model, test_dataloader, args, config, logger=None):
    ...
    with torch.no_grad():
        ...
        vote_times = int(getattr(args, 'test_vote_times', 299))
        if vote_times > 0:
            print_log(f"[TEST_VOTE]", logger=logger)
            acc = 0.
            for time in range(1, vote_times + 1):
