...
## 3D NEPA (ModelNet40, Query Interface)

This repo includes a 3D extension under `nepa3d/` that treats different 3D representations as a shared `query -> answer` interface and trains NEPA on token sequences.

Detailed implementation notes, experiment logs, and current result tables are tracked in:
...
```bash
bash nepa3d/data/download_modelnet40.sh
```

### Preprocess with PBS (recommended on cluster)
...
### Additional experiments now available

- Voxel backend (`BACKEND=voxel`) for third-representation transfer tests.
- Alternative pretrain objective baseline:
  - `OBJECTIVE=mae` with `MASK_RATIO=...`
- DDA quality analysis / sweep:
  - `scripts/analysis/nepa3d_dda_metrics.sh`
  - `scripts/analysis/nepa3d_dda_sweep.sh`
- Real point-cloud path (ScanObjectNN):
  - `scripts/preprocess/preprocess_scanobjectnn.sh`
  - `scripts/finetune/nepa3d_finetune_scanobjectnn.sh`

### Current status snapshot

- v1 cache is used (`data/modelnet40_cache_v1`) with pointcloud-ray DDA enabled.
- Multi-seed (`seed=0,1,2`) and from-scratch baseline runs are completed and summarized in `nepa3d/README.md`.
- MUST block is completed with val-selected 3x3 transfer and 3x3 linear-probe matrices.
- SHOULD block has results for voxel backend transfer and MAE objective baseline.
- DDA quality metrics were computed on test split (`results/dda_metrics_test.csv`).
- ScanObjectNN run is completed (pointcloud_noray, mesh-pretrained `n_types=5` ckpt):
  - `best_val=0.8906` (ep 98), `test_acc=0.8036`
  - details are summarized in `nepa3d/README.md`.
