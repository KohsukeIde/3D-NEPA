...
def _predict_sample(
    ...
    *,
    ...
    with torch.no_grad():
        pred = model.predict(ctx_t, qry_t)
...
def evaluate_control(
    ...
    *,
    ...
    for code in sorted(np.unique(src).tolist()):
        ...
        if int(mask.sum()) <= 0:
            continue
        row = _metrics(y_true[mask], y_pred[mask], tau=float(tau))
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("eval_udfdist_continuous_controls")
...
def main() -> None:
    ...
    for control in controls:
        ...
        overall = {
            ...
            f"iou@{float(args.tau):g}": sum(float(r[f"iou@{float(args.tau):g}"]) * int(r["n_tokens"]) for r in results)
            / float(max(1, overall_n)),
            "code_acc_proxy": sum(float(r["code_acc_proxy"]) * int(r["n_tokens"]) for r in results) / float(max(1, overall_n)),
            "majority_code_acc": sum(float(r["majority_code_acc"]) * int(r["n_tokens"]) for r in results)
            / float(max(1, overall_n)),
            "pred_mean": sum(float(r["pred_mean"]) * int(r["n_tokens"]) for r in results) / float(max(1, overall_n)),
            "pred_std": sum(float(r["pred_std"]) * int(r["n_tokens"]) for r in results) / float(max(1, overall_n)),
            "target_mean": sum(float(r["target_mean"]) * int(r["n_tokens"]) for r in results) / float(max(1, overall_n)),
            "target_std": sum(float(r["target_std"]) * int(r["n_tokens"]) for r in results) / float(max(1, overall_n)),
        ...
        for control, summary in by_control.items():
            ...
            deltas[control] = {
                "overall": {
                    ...
                    f"delta_iou@{float(args.tau):g}": float(summary["overall"][f"iou@{float(args.tau):g}"])
                    - float(baseline["overall"][f"iou@{float(args.tau):g}"]),
                    "delta_code_acc_proxy": float(summary["overall"]["code_acc_proxy"])
                    - float(baseline["overall"]["code_acc_proxy"]),
                }
            }
    payload = {
        "ckpt": str(args.ckpt),
