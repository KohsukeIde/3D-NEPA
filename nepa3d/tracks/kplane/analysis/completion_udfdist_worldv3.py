...
def _predict_distance(
    ...
    *,
    ...
    ctx_dist = torch.zeros((b, int(ctx_xyz.shape[1])), device=device, dtype=torch.float32)
    for s in range(0, n_total, qbs):
        ...
        with torch.no_grad():
            pred, *_ = model(ctx_xyz, ctx_dist, q_t)
        out[:, s:e] = pred.detach().cpu().numpy().astype(np.float32, copy=False)
    return out


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("completion_udfdist_worldv3")
                ...
                if int(args.mesh_eval) == 1 or export_assets:
                    try:
                        ...
                        if int(args.mesh_eval) == 1:
                            mm = mesh_metrics(
                                pred_mesh_vf[0], pred_mesh_vf[1], gt_levelset_vf[0], gt_levelset_vf[1],
                                n_samples=int(args.mesh_num_samples), taus=(float(tau_list[0]),),
                            )
