...
# Stage 2: wider order sweep after Stage 1 shows order/mask interaction is worth testing.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WORKDIR="${WORKDIR:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
ORDERS="${ORDERS:-simplified_morton,morton,fixed_random,axis_x,radial,farthest_greedy,bfs_shell,geodesic_shell,diffusion_shell}"
MASKS="${MASKS:-0.0,0.3,0.7}"
MAX_EPOCH="${MAX_EPOCH:-50}"
FT_MAX_EPOCH="${FT_MAX_EPOCH:-75}"
FT_SPLITS="${FT_SPLITS:-hardest}"
RUN_TAG="${RUN_TAG:-stage2_$(date +%Y%m%d_%H%M%S)}"
