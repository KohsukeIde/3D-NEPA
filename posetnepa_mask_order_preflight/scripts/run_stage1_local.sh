...
# Stage 1: diagnostic survey.
# This deliberately starts with mask-off + mask-on for baseline and one geometry order.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WORKDIR="${WORKDIR:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
ORDERS="${ORDERS:-simplified_morton,fixed_random,diffusion_shell}"
MASKS="${MASKS:-0.0,0.7}"
